...
export const AUTH_TOKEN_KEY = "logistic_copilot_auth_token";
...
export function LoginPanel({ hasSession, className = "" }: LoginPanelProps) {
  ...
  return (
    <div
      className={`rounded-[2rem] border border-white/12 bg-slate-950/72 p-6 shadow-2xl shadow-slate-950/40 backdrop-blur-xl sm:p-7 ${className}`}
    >
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-100">Secure access</p>
          <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-50">Sign in to dashboard</h2>
        </div>
      ...
      {hasSession ? (
        ...
        <form className="space-y-4" onSubmit={handleLogin}>
          <label className="block">
            <span className="text-xs font-medium uppercase tracking-[0.16em] text-slate-400">Business email</span>
          ...
          <label className="block">
            <span className="text-xs font-medium uppercase tracking-[0.16em] text-slate-400">Password</span>
            <div className="mt-2 flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-900/80 px-4 py-3 transition focus-within:border-cyan-300/60">
